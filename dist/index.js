...
var generatePrompt = (request, config) => `
...
var formatZodError = (error, label) => {
  const errorsString = error.errors.map((err) => {
    ...
    const prefix = path ? `${import_picocolors.default.cyan(path)}: ` : "";
...
`);
  ...
  static renderForTerminal(event, group) {
    ...
    let { message } = event;
    const groupIdentifiers = group ? group.getGroupIdentifiers() : [];
    let colorFn = import_picocolors2.default.white;
    switch (level) {
      case "error":
        colorFn = import_picocolors2.default.red;
        break;
      case "warn":
        colorFn = import_picocolors2.default.yellow;
        break;
      case "info":
        colorFn = import_picocolors2.default.cyan;
        break;
      case "debug":
        colorFn = import_picocolors2.default.green;
        break;
      case "trace":
        colorFn = import_picocolors2.default.gray;
        break;
    }
    if (event.level === "error") {
      message = import_picocolors2.default.red(message);
    }
    const outputParts = [];
    ...
    outputParts.push(...groupIdentifiers.map((name) => import_picocolors2.default.dim(name)));
    outputParts.push(message);
    if (parsedMetadata) {
      const formattedMetadata = typeof parsedMetadata === "string" ? parsedMetadata : Object.entries(parsedMetadata).map(([k, v]) => {
        ...
        return `${import_picocolors2.default.dim(k)}=${value}${typeof value === "string" && value.includes("\n") ? "\n" : ""}`;
      }).join(" ");
      outputParts.push(formattedMetadata);
    }
    const output = outputParts.join(" | ");
    if (event.level === "warn") {
      return import_picocolors2.default.yellowBright(output);
    }
    return output;
  }
};
...
var Log = class {
  ...
  outputEvent(event) {
    if (this.shouldLog(event.level)) {
      ...
      console.warn(
        import_picocolors3.default.bgYellowBright(import_picocolors3.default.black(" WARN ")),
        import_picocolors3.default.yellow(event.message)
      );
    }
  }
};
